...
public class CommandKillAll extends ICommand {
	...
	public void handleCommand(CommandSender cs, String[] args) {
		...
		int radius = -1;
		try{radius = Integer.parseInt(args[1]);} catch(Exception ex) {radius = -1;}
		if(type.equals("ALL")) {
			int count = killall(p, radius);
			...
			String msg = prefix + "Killed " + count + " of '" + et.name() + "' in a radius of " + radius;
			p.sendMessage(msg);
		}
	}
	
	private int killall(Player p, int r) {
		World w = p.getWorld();
